...
*/
{
  ...
  // 초기값
  dataWatcher.data = [
    { name: "인스타그램", percent: 45, icon: "instagram", color: "#B05BCE" },
    { name: "유튜브", percent: 90, icon: "youtube", color: "#C24343" },
    { name: "기업/단체", percent: 5, icon: "kin", color: "#6BB3DC" },
    { name: "트튀터", percent: 33.54, icon: "twitter", color: "#5754E7" },
    { name: "정부/공공", percent: 74.2, icon: "government", color: "#348E94" },
  ];
}
/*
■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
...
*/
{
  ...
  dataWatcher.callback(() => {
    ...
    $bars.forEach((_$bar, _idx) => {
      $cnts[_idx].innerText = dataWatcher.data[_idx];

      const ObserverClassIsStart = new window.ObserverClass($article, "is-start");
      ObserverClassIsStart.addCallback(() => {
        $bars[maxIndex].closest(".col").classList.add("col-is-active");
        _$bar.style.height = `${dataWatcher.data[_idx]}%`;
        _$bar.style.transitionDelay = `${_idx * 0.15}s`;
      });
  ...
  // 초기값
  dataWatcher.data = [22, 40, 44.1, 10.2, 100];
}
/*
■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
*/
